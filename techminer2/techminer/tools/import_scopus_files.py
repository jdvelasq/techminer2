...
>>> # techminer.tools.import_scopus_files(directory, disable_progress_bar=True)
...
--INFO-- Removing anonymous authors
...
def _remove_selected_document_types_from_documents(
    directory, **document_types
):
...
def _create__num_authors__column(directory):
    ...
    for file in files:
        ...
        data = data.assign(
            num_authors=data.num_authors.map(len, na_action="ignore")
        )
        data = data.assign(
    ...
    for file in files:
        ...
        if "raw_index_keywords" in data.columns:
            ...
            data.raw_index_keywords = data.raw_index_keywords.map(
                lambda x: sorted([y.strip() for y in x])
                if isinstance(x, list)
                else x
            )
            data.raw_index_keywords = data.raw_index_keywords.map(
...
def _process__raw_keywords__column(directory):
    ...
    for file in files:
        ...
        if (
            ...
            data.raw_keywords = data.raw_keywords.map(
                lambda x: pd.NA if x == [] else x
            )
...
def _process__raw_authors__column(directory):
    ...
    for file in files:
        ...
        if "raw_authors" in data.columns:
            ...
            data.raw_authors = data.raw_authors.str.replace(
                ",", ";", regex=False
            )
            data.raw_authors = data.raw_authors.str.replace(
                ".", "", regex=False
...
def _process__authors_id__column(directory):
    ...
    for file in files:
        ...
        if "document_type" in data.columns:
            data["authors_id"] = data.authors_id.str.replace(
                ";$", "", regex=True
            )
            data["authors_id"] = data.authors_id.map(
