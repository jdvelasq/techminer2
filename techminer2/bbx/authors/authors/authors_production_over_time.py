...
>>> from techminer2.bbx.authors.authors import authors_production_over_time
...
def authors_production_over_time(
    ...
    result = _Result()
    result.plot_ = production_over_time_chart(
        column="authors",
        top_n=top_n,
        directory=directory,
        title="Authors' Production over Time",
    )
    result.documents_per_author_ = column_documents(
        column="authors",
        directory=directory,
    )
