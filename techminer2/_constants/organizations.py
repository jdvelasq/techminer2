ORGANIZATIONS = frozenset(
    [
        "6th Regional Directorate of the General Directorate of Highways",
        "AC Ingenieria Virtual",
        "AC2 Systems Consultancy",
        "Academia China de las Ciencias Pekin",
        "Alcaldia de Itagui",
        "Alexander von Humboldt Foundation",
        "Almacenes Universales S.A.",
        "Aloe Technology S.A.S.",
        "Alpina Productos Alimenticios",
        "American Association of Pediatric Dentistry",
        "American College of Veterinary Internal Medicine",
        "Arboriente S.A.",
        "Argonne National Laboratory",
        "Asheville Buncombe Technical Community College",
        "Biofab",
        "Biofenix S.A.S",
        "Cabildo Catedral de Cordoba",
        "Caja Petrolera de Salud",
        "CarbonCure Technologies",
        "Cardiovascular Dynamic Research Group",
        "Ceiba Software",
        "Cementos Argos",
        "CENIDET",
        "CIATEC",
        "CICESE",
        "CICPBA",
        "CIEPLAN",
        "CINVESTAV",
        "CIUNR",
        "Colciencias",
        "COLPOS",
        "Comcel S.A.",
        "Comision Nacional de Seguridad",
        "Comision Nacional Del Agua",
        "CONACyT",
        "CONAHCYT",
        "Concejo Nacional de Ciencia y Tecnologia",
        "Concremag S.A.",
        "Concytec",
        "CONICET",
        "Consejo Nacional de Ciencia y Tecnologia",
        "Corporacion Nacional de Electricidad",
        "Corporacion Universitaria Americana",
        "Corporacion Universitaria Autonoma del Cauca",
        "Corporacion Universitaria del Caribe",
        "Corporacion Universitaria Lasallista",
        "Corporacion Universitaria Latinoamericana",
        "Corporacion Universitaria Minuto de Dios",
        "Corporacion Universitaria Reformada",
        "Corporacion Universitaria Remington",
        "Corporacion Universitaria Uniremington",
        "Corporacion, Universitaria Americana",
        "Defence Metallurgical Research Laboratory",
        "Departamento Nacional de Estadistica DANE",
        "Departamento Nacional de Estadistica",
        "Digital Media and Marketing Association",
        "EAFIT",
        "Ecopetrol",
        "Empresas Municipales de Cali",
        "Empresas Publicas de Medellin",
        "Energy Information Administration",
        "Esc. Sup. Politecnica del Chimborazo",
        "Escuela de Ingenieria de Bilbao",
        "Escuela de Marketing y Negocios",
        "Escuela Superior Politecnica de Chimborazo",
        "Escuela Superior Politecnica del Chimborazo",
        "Escuela Superior Politecnica del Litoral",
        "European Center for Medium Range Weather Prediction",
        "European Centre for Medium Range Weather Prediction",
        "Fraunhofer Chile Research",
        "Fundacao Getulio Vargas de Sao Paulo",
        "Fundacion Cardiovascular de Colombia",
        "Fundacion Catolica Lumen Gentium",
        "Fundacion Patagonia Natural",
        "Fundacion Universitaria Agraria de Colombia",
        "Fundacion Universitaria Autonoma de Las Americas",
        "Fundacion Universitaria Catolica del Norte",
        "Fundacion Universitaria CEIPA",
        "Fundacion Universitaria de ciencias de la salud",
        "Fundacion Universitaria de Popayan",
        "Fundacion Universitaria de San Gil",
        "Fundacion Universitaria Juan de Castellanos",
        "Fundacion Universitaria Los Libertadores",
        "Fundacion Universitaria Tecnologico Comfenalco",
        "GRENASER",
        "Grupo Cementos de Chihuahua",
        "Hospital Clinico San Carlos",
        "Hospital Luis Vernaza",
        "INIFAP",
        "Insitituto Tecnologico de Buenos Aires",
        "Institucion Educativa Pascual Bravo",
        "Institucion Universitaria Antonio Jose Camacho",
        "Institucion Universitaria CEIPA",
        "Institucion Universitaria CESMAG",
        "Institucion Universitaria de Envigado",
        "Institucion Universitaria ESUMER",
        "Institucion Universitaria ITM",
        "Institucion Universitaria ITSA",
        "Institucion Universitaria Pascual Bravo",
        "Instituto de Ciencias Agarias",
        "Instituto de Ciencias de la Educacion",
        "Instituto de Ciencias del Mar",
        "Instituto de Desarrollo Urbano de Bogota",
        "Instituto Federal de Santa Catarina",
        "Instituto Nacional de Tecnologia Agropecuaria",
        "Instituto Politecnico Nacional",
        "Instituto Potosino de Investigacion Cientifica y Tecnologica",
        "Instituto Tecnologico de Sonora",
        "Intel Corporation",
        "International Business Information Management Association",
        "Istituto Zooprofilattico Sperimentale dellUmbria e delle Marche",
        "ITTGTNM",
        "Ministry of Agriculture and Rural Affairs",
        "National Oceanic and Atmospheric Administration Environmental Modeling Center",
        "Nutrisolla",
        "Polytechnique Montreal",
        "Pontificia Universidad Catolica de Valparaiso",
        "Servicio Nacional de Aprendizaje (SENA)",
        "Servicio Nacional de Aprendizaje",
        "Sociedad Colombiana de Cardiologia y Cirugia Cardiovascular",
        "Tecnologico de Antioquia",
        "Tecnologico de Costa Rica",
        "Tecnologico de Estudios Superiores de Ecatepec",
        "Tecnologico de Monterrey",
        "Tecnologico Nacional de Mexico",
        "The Energy Information Administration",
        "The Food and Agriculture Organization",
        "The University of Hong Kong",
        "UANL",
        "UBioBio",
        "UNIAGUSTINIANA",
        "Unicartagena",
        "Unilasallista",
        "UNIMINUTO",
        "Uninorte",
        "UNISINOS",
        "Univ Autonoma de Chile",
        "Univ. Agraria del Ecuador",
        "Univ. Alberto Hurtado",
        "Univ. Autonoma Metropolitana Cuajimalpa",
        "Univ. Catolica de Cordoba",
        "Univ. Catolica de la Santisima Concepcion",
        "Univ. Catolica Luis Amigo",
        "Univ. Cooperativa de Colombia",
        "Univ. de Antioquia",
        "Univ. de Atacama",
        "Univ. de Concepcion",
        "Univ. de Cordoba",
        "Univ. de Narino",
        "Univ. Del Atlantico",
        "Univ. del Bio-Bio",
        "Univ. Del Norte",
        "Univ. Del Quindio",
        "Univ. Nac. del Nordeste",
        "Univ. Nac. Nordeste",
        "Univ. Nac. San Antonio",
        "Univ. Nacional de Colombia",
        "Univ. Nacional de Cordoba",
        "Univ. Pedagogica y Tecnologica de Colombia",
        "Univ. Peruana Cayetano Heredia",
        "Univ. Playa Ancha",
        "Univ. Tecn. Equinoccial",
        "Univ. Tecnica Federico Santa Maria",
        "Univeriste de Lorraine",
        "Universidad Agraria del Ecuador",
        "Universidad Alberto Hurtado",
        "Universidad Autonoma de Chile",
        "Universidad Autonoma Metropolitana Cuajimalpa",
        "Universidad Autonoma Metropolitana",
        "Universidad Catolica de Cordoba",
        "Universidad Catolica de la Santisima Concepcion",
        "Universidad Catolica Luis Amigo",
        "Universidad de Antioquia",
        "Universidad de Atacama",
        "Universidad de Cartagena",
        "Universidad de Concepcion",
        "Universidad de Narino",
        "Universidad Del Atlantico",
        "Universidad del Bio-Bio",
        "Universidad del Norte",
        "Universidad Del Quindio",
        "Universidad del Valle",
        "Universidad EIA",
        "Universidad Nacional de Colombia",
        "Universidad Nacional de Cordoba",
        "Universidad Nacional San Antonio",
        "Universidad Peruana Cayetano Heredia",
        "Universidad Playa Ancha",
        "Universitei libre de Bruxelles",
        "University of Oviedo",
        "Univesidad de Los Andes",
        "Univresidad de Antioquia",
        "VABIOTECH",
        "Western Carolina University",
        "Xijing Hospital",
        "XM S.A E.S.P",
        "Latvijas Universitāte",
        "Hitachi, Ltd.",
        "Indian Institute of Management",
    ]
)
