...
ALL_NOUN_PHRASES = None
STOPWORDS = None
ALL_KEYWORDS = None
CONNECTORS = None
COPYRIGHT_REGEX = None
KNOWN_NOUN_PHRASES = None
DISCURSIVE_PATTERNS = load_text_processing_terms("discursive_patterns.txt")
TEXT_NOUN_PHRASES = None


# ------------------------------------------------------------------------------
def _load_all_records_from_database(root_directory):
    params = Params(root_directory=root_directory)
    records = load_all_records_from_database(params)
    return records


# ------------------------------------------------------------------------------
def _write_records_to_database(dataframe, root_directory):
    params = Params(root_directory=root_directory)
    write_records_to_database(params=params, records=dataframe)


# ------------------------------------------------------------------------------
def _collect_all_keywords(dataframe):
...
# ------------------------------------------------------------------------------
def _collect_all_noun_phrases(dataframe, dest):
...
# ------------------------------------------------------------------------------
def _load_known_noun_phrases():
    phrases = load_text_processing_terms("known_noun_phrases.txt")
    phrases = [phrase.lower().replace("_", " ") for phrase in phrases]
    return phrases


# ------------------------------------------------------------------------------
def _extract_acronyms_from_text(text):
    #
...
# ------------------------------------------------------------------------------
def _clean_key_terms(stopwords, key_terms):
...
# ------------------------------------------------------------------------------
def _mark_copyright_text(copyright_regex, text):
...
# ------------------------------------------------------------------------------
def _mark_discursive_patterns(discursive_patterns, text):
...
# ------------------------------------------------------------------------------
def _mark_connectors(connectors, text):
...
# ------------------------------------------------------------------------------
def _join_consequtive_separate_terms_in_uppercase(text, stopwords):
...
# ------------------------------------------------------------------------------
def _highlight_key_terms(key_terms, text):
    key_terms = sorted(
        key_terms,
...
# ------------------------------------------------------------------------------
def _unmark_lowercase_text(text):
...
# ------------------------------------------------------------------------------
def _remove_roman_numbers(text):
    roman_numbers = [
        "i",
        "ii",
        "iii",
...
# ------------------------------------------------------------------------------
def _make_final_corrections(text):
...
# ------------------------------------------------------------------------------
def _replace_urls(text, matches):
    if len(matches) == 0:
        return text
    for match in matches:
...
def _transform_email_addresses_to_lower_case(text):
...
# ------------------------------------------------------------------------------
def _mark_template_abstract_separators(text):
...
# ------------------------------------------------------------------------------
def _unmark_template_abstract_markers(text):
...
def _process_column(text, ctx):
    ...
    # Algorithm:
    #
    key_terms += [k for k in ctx["all_noun_phrases"] if k in text]
    found_noun_phrases = list(key_terms)
    key_terms += _extract_acronyms_from_text(text)  # <----------------
    key_terms = _clean_key_terms(ctx["stopwords"], key_terms)
    key_terms += [k for k in ctx["all_keywords"] if k in text]
    key_terms += [k for k in ctx["known_noun_phrases"] if k in text]

    url_matches = _collect_urls(text)  ###

    text = _mark_copyright_text(ctx["copyright_regex"], text)  ###
    text = _mark_template_abstract_separators(text)
    text = _mark_discursive_patterns(ctx["discursive_patterns"], text)
    text = _mark_connectors(ctx["connectors"], text)

    #
    #
    text = _highlight_key_terms(key_terms, text)
    #
    #

    text = _repair_appostrophes(text)

    text = _join_consequtive_separate_terms_in_uppercase(text, ctx["stopwords"])

    text = _fix_units(text)

    text = _replace_urls(text, url_matches)

    text = _unmark_lowercase_text(text)
    text = _unmark_template_abstract_markers(text)
    text = _unmark_et_al(text)
    text = _mark_connectors(ctx["connectors"], text)
    text = _remove_roman_numbers(text)

    text = _transform_email_addresses_to_lower_case(text)

    text = _make_final_corrections(text)

    return (text, found_noun_phrases)


# ------------------------------------------------------------------------------
def highlight_descriptors(
    source: str,
    target: str,
    root_directory: str,
) -> int:

    dataframe = _load_all_records_from_database(root_directory)

    if source not in dataframe.columns:
        return 0

    dataframe[target] = dataframe[source].copy()

    all_keywords = _collect_all_keywords(dataframe)
    frequent_keywords, all_keywords = _clean_all_keywords(all_keywords)

    all_noun_phrases = _collect_all_noun_phrases(dataframe, target)
    known_noun_phrases = _load_known_noun_phrases()

    connectors = load_text_processing_terms("connectors.txt")
    ...
    determiners = load_text_processing_terms("determiners.txt")
    determiners = (
        "(" + "|".join(["^" + determiner + r"\s" for determiner in determiners]) + ")"
    )
    determiners = re.compile(determiners)

    # Prepare context to pass to _process_column to avoid module globals
    ctx = {
        "all_noun_phrases": all_noun_phrases,
        "stopwords": stopwords,
        "all_keywords": all_keywords,
        "connectors": connectors,
        "copyright_regex": copyright_regex,
        "known_noun_phrases": known_noun_phrases,
        "discursive_patterns": DISCURSIVE_PATTERNS,
    }

    with stdout_to_stderr():

        results = dataframe[target].parallel_apply(partial(_process_column, ctx=ctx))

    # results: Series of (processed_text, noun_phrases_list)
    dataframe[target] = results.apply(lambda r: r[0] if isinstance(r, tuple) else r)

    text_noun_phrases = []
    for r in results:
        if isinstance(r, tuple) and r[1]:
            text_noun_phrases.extend(r[1])

    _write_records_to_database(dataframe, root_directory)

    _report_undetected_keywords(
        frequent_keywords,
