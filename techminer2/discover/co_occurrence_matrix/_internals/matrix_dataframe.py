...
Smoke tests:
    ...
    ...     #
    ...     # COLUMNS:
    ...     .with_column_field(CorpusField.AUTH_KEY_RAW)
    ...
    ...     #
    ...     # ROWS:
    ...     .with_index_field(None)
    ...     .having_index_items_in_top(None)
    ...     .having_index_items_ordered_by(None)
...
class MatrixDataFrame(
    ...
    def _step_01_check_row_params(self):
        if self.params.index_field is None:
            self.with_other_field(
                self.params.field,
            )
            self.having_index_items_in_top(
                self.params.top_n,
            )
            self.having_index_items_ordered_by(
                self.params.items_order_by,
            )
            self.having_index_item_occurrences_between(
                self.params.item_occurrences_range[0],
                self.params.item_occurrences_range[1],
            )
            self.having_index_item_citations_between(
                self.params.item_citations_range[0],
                self.params.item_citations_range[1],
            )
            self.having_index_items_in(
    ...
    def _step_05_create_raw_matrix_list(self, records):
        #
        columns = self.params.source_field
        rows = self.params.index_field
        #
