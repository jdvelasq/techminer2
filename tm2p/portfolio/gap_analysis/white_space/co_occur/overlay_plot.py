...
.. raw:: html
    ...
    <iframe src="../_generated/px.portfolio.gap_analysis.white_space.co_occur.overlay_plot.html"
...
Smoke tests:
    ...
    >>> from tm2p.portfolio.gap_analysis.white_space.co_occur import OverlayPlot  # type: ignore
    >>> fig = (
    ...     OverlayPlot()
    ...     #
    ...     # ANALYSIS UNIT:
    ...
    ...     .using_uniform_edge_color("#a9a9a9")
    ...     #
    ...
    >>> fig.write_html("docsrc/_generated/px.portfolio.gap_analysis.white_space.co_occur.overlay_plot.html")


"""

from tm2p._intern import ParamsMixin
from tm2p._intern.plots.adv.wh_overlay_netw_plot import build_wh_overlay_network_plot
from tm2p.portfolio.thematic_struct.co_occur.direct import DirectMatrix
from tm2p.portfolio.thematic_struct.co_occur.direct import Matrix as CoOccurrenceMatrix
from tm2p.portfolio.thematic_struct.co_occur.direct import UnitToCluster

from .matrix import Matrix as WhiteSpaceMatrix
...
class OverlayPlot(
    ...
    def run(self):
        ...
        white_space_matrix = (
            WhiteSpaceMatrix().update(**self.params.__dict__).using_counters(True).run()
        )

        i2c = UnitToCluster().update(**self.params.__dict__).using_counters(True).run()

        fig = build_wh_overlay_network_plot(
            params=self.params,
