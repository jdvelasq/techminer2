...
class BaseContainsMatch(
    ...
    def run(self) -> str:
        ...
        thesaurus_df = _contains(thesaurus_df=thesaurus_df, params=self.params)
...
def _contains(thesaurus_df: pd.DataFrame, params: Params) -> pd.DataFrame:
    return thesaurus_df[  #  type: ignore
        thesaurus_df[PREFERRED].str.contains(f"{params.pattern[0]}")
    ].copy()
