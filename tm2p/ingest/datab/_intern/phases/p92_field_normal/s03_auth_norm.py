...
def s03_auth_norm(root_directory: str) -> int:
...
def _build_author_mapping(root_directory: str) -> dict[str, str]:
    ...
    df = load_main_csv_zip(
        root_directory,
        usecols=[Field.AUTH_NORM.value, Field.AUTHID.value],
    )
    df = df.dropna()

    df[Field.AUTH_NORM.value] = df[Field.AUTH_NORM.value].str.split("; ")
    df[Field.AUTHID.value] = df[Field.AUTHID.value].str.split("; ")

    for _, row in df.iterrows():
        if len(row[Field.AUTH_NORM.value]) != len(row[Field.AUTHID.value]):
            raise ValueError(
    ...
    df[Field.AUTHID.value] = df[Field.AUTHID.value].str.strip()

    df = df.drop_duplicates(subset=[Field.AUTHID.value])
    ...
    return dict(
        zip(
            df[Field.AUTHID.value].str.replace(";", "").str.strip(),
